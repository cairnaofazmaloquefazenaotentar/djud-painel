...
export async function GET(request: NextRequest) {
  try {
    ...
    if (anoMin || anoMax) {
      where.anoBase = {};
      if (anoMin) where.anoBase.gte = anoMin;
      if (anoMax) where.anoBase.lte = anoMax;
    }
    ...
    console.error("[precos/bps]", error);
    return NextResponse.json({ error: "Erro ao consultar BPS" }, { status: 500 });
  }
}
